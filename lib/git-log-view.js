...
GitLogView.prototype.get_log = function() {
    ...
    var settings = atom.config.getSettings();
    var maxCommits = settings['git-log'].maxCommitCount;
    var args = ['log', '--decorate=full', '--date=default', '--pretty=fuller', '--all', '--parents', '--numstat', '--topo-order', '--raw', '--max-count=' + maxCommits];
    var options = {};
