    ...
    main_panel.table.on("click", "p", (function(self) {
        return function(e) {
            ...
            if(commit_data.refs.length > 0)
                self.info_panel.add_content("Labels:", commit_data.refs.join(","));
            self.info_panel.append('<p>' + safe_tags(commit_data.message) + "</p>");
            self.info_panel.append('<img src="' + safe_tags(self.get_image(commit_data.author_email)) + '"/>')
        };
    })(this));
}
