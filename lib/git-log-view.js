...
GitLogView.prototype.fill_content = function() {
    ...
    /*main_panel.body.on("click", "p", (function(self) {
        return function(e) {
            console.log('yooo')
            ...
            self.info_panel.info_file.body.empty();
...
main_panel.body.on("click", "tr", (function(self) {
        return function(e) {
            ...
            self.info_panel.info_data.empty();
            self.info_panel.info_image.empty();
            self.info_panel.body.empty();
            var line_no = $(this).index();
            var commit_data = self.log[line_no];

            // background line higlighting
            if(self.previous_line != null)
                self.main_panel.body.find('tr:nth-child(' + self.previous_line + ')')
                    .css('background-color', '');

            self.previous_line = line_no + 1;
            $(this).css('background-color', 'rgba(13,128,215,0.65)');
            ...
            for(i=1; len=commit_data.file_line_diffs.length, i<len;i++) {
                ...
                self.info_panel.body.append(temp);
            }
        };
    })(this));
