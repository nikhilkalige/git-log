...
GitLogView.prototype.initialize = function(repo_name, commit_skip, current_file) {
    ...
    this.get_log(commit_skip, settings['git-log'].commitLimit);
...
GitLogView.prototype.get_log = function(skip_no, count) {
    this.log = null;
...
**/
    ...
    this.main_panel.button.on('click', (function(self) {
        return function(e) {
            console.log('asdfasdf');
            self.main_panel.body.empty();
            self.main_panel.graph.empty();
            self.get_log(50, atom.config.getSettings()['git-log'].commitLimit);
        };
    })(this));
};
...
GitLogView.prototype.parser = function() {
    ...
    var graph = new GitGraph(this.main_panel.graph, this.log, this.line_height, 2, this.branches);
};
...
GitLogView.prototype.select_display_info = function(target, line_no) {
    ...
    target.css('background-color', 'rgba(13,128,215,0.65)');
