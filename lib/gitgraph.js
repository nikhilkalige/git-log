...
function GitGraph(location, data, line_height) {
    this.config = this.set_config(line_height);
...
GitGraph.prototype.render = function(location, data) {
    ...
    //var svg = d3.select(location.get(0)).append("svg");
    var svg = location.append("<svg>").find("svg");
    svg.attr("xmlns", "http://www.w3.org/2000/svg")
    .attr("xmlns:xlink", "http://www.w3.org/1999/xlink")
    //var line_group = svg.append("g");
    //var circle_group = svg.append("g");;

    var line_group = svg.append("<g>").find("g:first");
    var circle_group = svg.append("<g>").find("g:last");
    ...
    data.forEach(function(d) {
        var circle = circle_group.append("<circle>").find("circle:last");
        circle.attr("cx", (self.config.left_margin + (d.position.column * self.config.branch_spacing)))
            .attr("cy", (self.config.top_margin + (d.position.row * self.config.circle_spacing)))
            .attr("r", self.config.circle.radius)
            .attr("fill", d.position.color);
    })
...
*/
    line_array.forEach(function(d) {
        var path = line_group.append("<path>").find("path:last");
        path.attr("d", create_line(d))
            .attr("stroke", d[0].color);
    })

    /*return svg.attr("width", svg[0][0].childNodes[1].getBoundingClientRect().width + this.config.left_margin)
              .attr("height", height);*/
    return svg.attr("width", svg[0].getBoundingClientRect().width + this.config.left_margin)
        .attr("height", height);
};
