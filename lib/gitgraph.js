...
function GitGraph(location, data, line_height, margin, branches) {
    var branches_local = branches.slice(0);
    this.config = this.set_config(line_height, margin);
    this.set_position(data, branches);
    console.time('render');
    var svg = this.render(location, data, branches_local);
...
GitGraph.prototype.set_position = function(data, branches) {
    var _i, _len;
...
GitGraph.prototype.commit_search = function(data, commit, start_index) {
    var i, len;
    for(i = start_index; len = data.length, i < len; i++) {
        if(data[i].sha1 === commit)
            return i;
    }
};

GitGraph.prototype.lines = function(data, branches) {
    ...
    for(i=0; len = branches.length, i < len; i++) {
        ...
        var index = this.commit_search(data, branches[i], 0);
        ...
        if(parent !== undefined) {
            ...
            end.y = this.config.commit_count - 1;
        }
    ...
    for(i=0; len = data.length, i < len; i++) {
        ...
        for(j=0; _len = commit.parents.length, j < _len; j++) {
            ...
            var index = this.commit_search(data, commit.parents[j], i);
            var parent = data[index];
...
GitGraph.prototype.render = function(location, data, branches) {
    console.time('render1');
    var line_array = this.lines(data, branches);
